 ...
 %%   
    ...
    for e=1:8
        figure(1); subplot(4,2,e); 
         ...
         title(['mean y=',num2str(mean(MP05_ons{e,2}),3), '  std y=',num2str(std(MP05_ons{e,2}),3) ]);
    end
   
       figure(2); clf;  colordef black; set(gcf,'color','k');
    for e=1:8
        figure(2); subplot(4,2,e); 
         hist(MP10_ons{e,2},x);
         set(findobj(gca,'Type','patch'),'FaceColor','y');
         hold on; hist(MP10_ons{e,1},x);  xlim([0,450]);
          title(['mean y=',num2str(mean(MP10_ons{e,2}),3), '  std y=',num2str(std(MP10_ons{e,2}),3) ]);
    end
